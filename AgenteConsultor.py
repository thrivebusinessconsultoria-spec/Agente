...
# SEGURANÇA CORRIGIDA: A chave do Resend deve vir apenas de variáveis de ambiente.
...
# ==============================================================================
#  PERSONALIDADES E BASE DE CONHECIMENTO (SISTEMA ESPECIALISTA)
# ==============================================================================
...
KNOWLEDGE_BASE = {
    "Estratégia e Direção": {
        ...
        "acao": "Definição de OKRs Trimestrais", "obj": "Alinhamento total da equipe", "res": "Foco laser nas prioridades"
    },
    "Gestão Financeira": {
        "impl": "Cegueira gerencial. Sem dados financeiros precisos, qualquer decisão de crescimento é uma aposta de alto risco de ruína.",
        "causas": [ "Mistura patrimonial (Caixa da Empresa vs Pessoal).", "Falta de DRE Gerencial mensal.", "Desconhecimento da Margem de Contribuição real." ],
        "acao": "Segregação Patrimonial Imediata", "obj": "Blindar o caixa da empresa", "res": "Clareza real do lucro líquido"
    },
    "Operação e Processos": {
        "impl": "O dono é o gargalo. A centralização operacional impede a escala e gera inconsistência na entrega final, limitando a alavancagem.",
        "causas": [ "Processos tribais (estão na cabeça das pessoas).", "Falta de Padrões Operacionais (POPs).", "Retrabalho constante por falha de comunicação." ],
        "acao": "Mapeamento do Processo Crítico", "obj": "Retirar o dono da operação", "res": "Autonomia da equipe e padrão"
    },
    "Vendas e Receita": {
        "impl": "Vendas por 'sorte' ou indicação. A falta de previsibilidade de receita impede investimentos seguros e compromete a performance.",
        "causas": [ "Ausência de Funil de Vendas estruturado.", "Dependência passiva de indicações.", "Falta de roteiro (script) de vendas." ],
        "acao": "Implantação de CRM Básico", "obj": "Gestão visual do pipeline", "res": "Previsibilidade de fechamentos"
    },
    "Pessoas e Gestão de Talentos": {
        "impl": "Baixa performance crônica. Sem liderança clara, a cultura se deteriora e os talentos vão embora, afetando o propósito.",
        "causas": [ "Contratação baseada apenas em 'feeling'.", "Ausência de rituais de feedback (1:1).", "Falta de descritivo de cargos (R&S)." ],
        "acao": "Instituir Rituais de Feedback 1:1", "obj": "Alinhamento de expectativas", "res": "Retenção e engajamento"
    },
    "Jurídico e Conformidade": {
        "impl": "Risco de Ruína invisível. Passivos trabalhistas ou contratuais podem destruir anos de lucro em semanas, comprometendo a blindagem.",
        "causas": [ "Informalidade nas contratações (PJ 'disfarçado').", "Contratos com clientes mal redigidos.", "Sócios sem Acordo de Quotistas." ],
        "acao": "Audit de Contratos Críticos", "obj": "Mapear riscos explosivos", "res": "Segurança jurídica e blindagem"
    },
    "Tecnologia e Dados": {
        "impl": "Ineficiência manual. A equipe gasta tempo em tarefas repetitivas que poderiam ser automatizadas, gerando gargalo operacional.",
        "causas": [ "Uso excessivo de papel ou planilhas desconexas.", "Sistemas que não conversam (Silos de dados).", "Baixa segurança da informação." ],
        "acao": "Centralização de Dados (SSOT)", "obj": "Eliminar duplicidade de dados", "res": "Velocidade na tomada de decisão"
    }
}
...
# O MANUAL OPERACIONAL (Usado no System Prompt)
SYSTEM_INSTRUCTION = """
VOCÊ É O CONSULTOR SÊNIOR DA THRIVE BUSINESS.

1. IDENTIDADE E VOZ
- Autoridade Técnica: Baseie-se no MDMP. Seja assertivo, nunca hipotético.
...
Use sempre: "Gargalo Crítico", "Alavancagem Estratégica", "Diagnóstico Cirúrgico", "MDMP", "Ações Táticas Imediatas", "Performance e Propósito", "Sustentabilidade Sistêmica".
Se Classificação = Sobrevivência, use: "Risco de Ruína".
...
**Implicação Primária:** (Qual o impacto central na empresa?)

### 3 Principais Causas Raiz
- [Causa 1 - Sistêmica]
- [Causa 2 - Processual]
- [Causa 3 - Comportamental/Gestão]
...
def analyze_cross_patterns(scores_map: Dict[str, float], perfil: dict) -> List[Dict[str, str]]:
    ...
    # Lógica 1: Penalidade por porte (Risco Sistêmico)
    ...
    if vend >= 5.0 and fin <= 4.0:
        insights.append({
            ...
            "analise": "O esforço comercial é alto, mas a margem é baixa. O problema não é o vendedor, é a precificação de custos.",
...
def apply_score_penalty(data: Dict[str, float], perfil: dict) -> Dict[str, float]:
    """Aplica penalidade na escala 1-3 se o porte for grande e o problema for de gestão."""
    ...
    if perfil["nivel_exigencia"] == "crítico":
        for k in scores_ajustados.keys():
            if "Pessoas" in k or "Operação" in k or "Jurídico" in k:
                # Se a nota for abaixo de 2.5 (escala 1-3), penaliza 15%
...
def get_classification(score_0_10: float) -> str:
    """Classifica o score normalizado para 0-10."""
...
def analyze_data(scores: MDMPScore):
    ...
    # 1. Contexto do Cliente e Penalidade (Camada 1)
    ...
    return {
        ...
        "scores_normalizados_0_10": normalized_data_ajustado # Dados para a IA
    }

# ==============================================================================
#  GERAÇÃO DE RELATÓRIO (HÍBRIDA: IA OU SISTEMA ESPECIALISTA)
# ==============================================================================

def generate_fallback_report(analise: dict) -> str:
    """
    SISTEMA ESPECIALISTA DETERMINÍSTICO (Modo Offline).
    Utiliza a KNOWLEDGE_BASE para gerar um relatório estruturado.
    """
    gargalo_nome = analise['gargalo']
    ...
    # Identifica o 2º pilar mais fraco para os Próximos Passos
...
def generate_report(analise: dict, cliente_nome: str):
    """Tenta gerar com IA, mas obriga-a a seguir as decisões do Python e usa fallback robusto."""
    fallback_text = generate_fallback_report(analise)
    
    if not GEMINI_API_KEY:
        return fallback_text, "Modo Especialista (Offline)"

    # PROMPT BLINDADO (Garante que a IA use os dados processados)
    prompt = f"""
    ...
    - Ação Principal (Fase 1): "{analise['acao_macro']}"
    ...
    try:
        ...
        if response.status_code == 429:
             logging.warning("Quota Gemini excedida. Usando Fallback.")
             return fallback_text, "Modo Especialista (Quota Excedida)"
...
def diagnose(scores: MDMPScore):
    ...
    # Retorna o score normalizado 0-10 e a classificação
    return {
        "status": modo, 
